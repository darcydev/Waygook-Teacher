...
<script
  src="https://www.paypal.com/sdk/js?client-id=ARvfpMvKkiNJfbgwGh4WW6x4temHSHbz9U-pkUbcbPAtibI64cb9GFMuCgqfFHPooxOGRGgfewGSi3oz">
</script>
<script>
...
paypal.Buttons({
	...
	** */
	...
	onApprove: function(data, actions) {
		...
		return actions.order.capture().then(function(details) {
			...
			return fetch('http://127.0.0.1:8888/Waygook-Teacher/paypal/get-transaction.php', {
				method: 'post',
				headers: {
					'content-type': 'application/json'
				},
	...
	** */
	/* **
		...
		** it creates the transaction on the Server-side
		** returns a success alert message
		from here: https://bit.ly/2Iqitnn
	** */
	onApprove: function(data, actions) {
  		return actions.order.authorize().then(function(authorization) {
    		...
    		return fetch('http://127.0.0.1:8888/Waygook-Teacher/paypal/capture-authorization.php', {
      			headers: {
        			'content-type': 'application/json'
      			},
      			body: JSON.stringify({
